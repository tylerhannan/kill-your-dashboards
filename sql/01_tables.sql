...
--    takes most of that back. Section 11 of 10_verify.sql measures the
...
-- ---------------------------------------------------------------------
-- agent_traces -- agent observability, on the same engine as the data
-- the agents are querying.
--
-- Every tool call an agent makes lands here. That means "why did this
-- agent give a wrong answer" and "why did the sportsbook lag last
-- night" are answered with the same primitives, in the same place, at
-- the same speed. The alternative is a second stack, which is the split
-- that produces missing context.
-- ---------------------------------------------------------------------
DROP TABLE IF EXISTS igaming.agent_traces;
CREATE TABLE igaming.agent_traces
(
    trace_id            UUID,
    span_id             UUID,
    ...
    -- The one natural language question that started the trace. Many
    -- spans share it; that ratio is the fan-out number.
    user_question       String CODEC(ZSTD(3)),
    -- Which turn of the agent loop this span belongs to.
    loop_iteration      UInt8 CODEC(ZSTD(1)),

    tool_name           LowCardinality(String),
        -- run_select_query | list_tables | describe_table | list_databases
    -- Literals stripped, so repeated query shapes collapse into one
    -- fingerprint and the redundant fan-out becomes countable.
    sql_fingerprint     String CODEC(ZSTD(3)),
    target_table        LowCardinality(String),

    duration_ms         UInt32 CODEC(ZSTD(1)),
    -- Time the query spent waiting for a slot rather than executing.
    -- Under fan-out this is the number that moves first.
    queue_wait_ms       UInt32 CODEC(ZSTD(1)),
    rows_read           UInt64 CODEC(ZSTD(1)),
    bytes_read          UInt64 CODEC(ZSTD(1)),
    result_rows         UInt32 CODEC(ZSTD(1)),
    peak_memory_bytes   UInt64 CODEC(ZSTD(1)),

    tokens_in           UInt32 CODEC(ZSTD(1)),
    tokens_out          UInt32 CODEC(ZSTD(1)),

    status              LowCardinality(String),   -- ok | error | timeout | truncated
    error_kind          LowCardinality(String),   -- '' when ok
    -- Set when the result was cut short and the model was handed a
    -- partial answer without being told. This is where confident
    -- mistakes are born.
